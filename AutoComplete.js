...
const ls = {
  ...
  execDir(path, options) {
    ...
    rawFiles = rawFiles.sort(function (a, b) {
      const aFileName = util.path.file(a.file)
        .trim().toLowerCase().replace(/\W/g, '');
      const bFileName = util.path.file(b.file)
        .trim().toLowerCase().replace(/\W/g, '');
...
function Exp(options) {
  ...
  this.data = function (string) {
    if (self.options) {
      ...
      if (typeof self.options.startDirectory === "string") {
        string = self.options.startDirectory + string;
      }
    }
